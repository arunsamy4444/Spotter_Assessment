  ...
  if (selectedFlightId) {
    ...
    return (
      <div className="flight-details">
        ...
        <p><strong>Airline:</strong> {leg.airline?.name || "Unknown Airline"}</p>

        <p>
          <strong>From:</strong> {leg.origin?.name || "Unknown Origin"} ({leg.origin?.id || "N/A"})
        </p>

        <p>
          <strong>To:</strong> {leg.destination?.name || "Unknown Destination"} ({leg.destination?.id || "N/A"})
        </p>

        <p>
          <strong>Departure:</strong> {leg.departure ? new Date(leg.departure).toLocaleString() : "N/A"}
        </p>

        <p>
          <strong>Arrival:</strong> {leg.arrival ? new Date(leg.arrival).toLocaleString() : "N/A"}
        </p>

        <p><strong>Duration:</strong> {leg.duration || "N/A"}</p>

        <p><strong>Cabin Class:</strong> {flight.cabinClass || "N/A"}</p>

        <p><strong>Price:</strong> ${flight.price?.raw?.toFixed(2) || "N/A"}</p>

        <p>
          {flight.bookingLink ? (
            <a href={flight.bookingLink} target="_blank" rel="noreferrer">
              Book this flight
            </a>
          ) : (
  ...
  const filteredFlights = flights
    ...
    .filter((flight) => {
      ...
      const dateMatch = filters.date
        ? new Date(flight.departure_at).toISOString().split("T")[0] === filters.date
        : true;
          ...
          filteredFlights.map((flight) => (
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div>
                  <strong>{flight.from} ({flight.from_city})</strong> → <strong>{flight.to} ({flight.to_city})</strong>
                </div>
                <div style={{ fontWeight: "600", color: "#555" }}>{flight.airline}</div>
              </div>

              <div style={{ marginTop: 6 }}>
                <span><strong>Departure:</strong> {new Date(flight.departure_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                {" — "}
                <span><strong>Arrival:</strong> {new Date(flight.arrival_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                {" | "}
